...
- **GitLab (namespace gitlab)**: GitLab CE, Runners, Registry com backend S3, Redis, RabbitMQ
- **Observability (namespace observability)**: OTEL Collector → Prometheus/Loki/Tempo → Grafana
...
- **Serviços Gerenciados AWS**: RDS PostgreSQL Multi-AZ
...
**Custo durante testes (uso intermitente - 8h/dia útil):**
- EKS Control Plane: $73/mês (sempre ligado)
- 3-4 EC2 nodes ligados 8h/dia: ~$120/mês
- RDS db.t3.small (ligado sob demanda): ~$40/mês
- EBS volumes + S3 + Rede: ~$30/mês
- **TOTAL DESENVOLVIMENTO**: ~$260-280/mês USD (~R$ 1.560-1.680/mês)

**Custo infraestrutura "idle" (cluster respirando 24/7):**
- EKS Control Plane: $73/mês
- 2 nodes mínimos t3.medium (system): ~$60/mês
- RDS db.t3.micro stopped: $0/mês (apenas storage ~$12/mês)
- Volumes EBS persistentes: ~$15/mês
- **TOTAL IDLE**: ~$160/mês USD (~R$ 960/mês)

💡 **Economia**: Desligar nodes fora do horário de trabalho e usar RDS stopped/started manualmente reduz custos em ~45%.

**Obs:** Conversão USD → BRL calculada com cotação aproximada de R$ 6,00 (referência jan/2026). Valores sujeitos a variação cambial.

## Observações

**Contexto de Desenvolvimento:**
- Este ambiente é para desenvolvimento/teste. Recursos podem ser ligados/desligados conforme necessário.
- Testes end-to-end completos devem ser executados apenas quando todos os componentes estiverem disponíveis.
- Aplicação Python de exemplo para validação de CI/CD: usar FastAPI com conexões a Postgres/Redis/RabbitMQ para testar pipeline completo.
- Acesso externo ao RDS: configurar via bastion host ou VPN quando necessário para troubleshooting (pgAdmin, DBeaver).
- Redis e RabbitMQ acessíveis via port-forward do Kubernetes para troubleshooting local.
