...
# ---------------------------------------------------------------------------
# Data sources: resolve GitLab and Keycloak ALB ARNs dynamically.
# These ALBs are created by the AWS Load Balancer Controller (not by TF),
# so we look them up by name. Names are stable (generated by AWS LBC from
# the Ingress group.name annotation hash).
#
# ALB names (from AWS Console / aws elbv2 describe-load-balancers):
#   GitLab:   k8s-gitlabstaging-da5a4e8c6d  (ingress group: gitlab-staging)
#   Keycloak: k8s-stagingp-keycloak-0dbafff841 (no group.name, uses ns/ingress-name)
#
# WAF associations were previously done via AWS CLI (2026-03-18):
#   aws wafv2 associate-web-acl --web-acl-arn <acl> --resource-arn <alb>
# Codified here for zero drift compliance.
# ---------------------------------------------------------------------------

data "aws_lb" "gitlab_alb" {
...
module "waf_staging" {
  ...
  # Primary ALB — platform-staging (platform ingress group)
