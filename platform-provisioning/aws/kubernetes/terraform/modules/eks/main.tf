# EKS Module for Observability Platform
# Follows ADR-002: Single EKS cluster with namespace isolation
...
resource "aws_eks_node_group" "main" {
  ...
  node_group_name = "${var.project_name}-${each.key}-ng"
  ...
  tags = {
    Name      = "${var.project_name}-${each.key}-ng"
    Project   = var.project_name
    ManagedBy = "terraform"
  }
...
# If the profile is localstack, don't try to create the OIDC provider
# as it's handled differently and often not needed in the same way.
