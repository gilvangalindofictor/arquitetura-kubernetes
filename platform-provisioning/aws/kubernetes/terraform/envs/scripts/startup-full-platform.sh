...
kubectl get pods -n test-apps 2>/dev/null || echo "   ℹ️  Test Applications não encontradas"
kubectl get ingress -n test-apps 2>/dev/null || echo "   ℹ️  Nenhum Ingress em test-apps"
...
echo "✅ Marco 2 Fase 4: Loki + Fluent Bit (Logging centralizado)"
...
echo "✅ Marco 2 Fase 7: Test Applications (nginx-test + echo-server, 2 ALBs HTTP-only)"
